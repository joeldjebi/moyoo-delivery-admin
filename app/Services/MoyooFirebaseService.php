...
class MoyooFirebaseService
{
    ...
    private function createJWT(): string
    {
        ...
        if (!$serviceAccountKey || !isset($serviceAccountKey['private_key'])) {
            throw new Exception('Service Account Key non configuré');
        }
    ...
    private function sendRequest(array $payload): array
    {
        try {
            ...
            // Log de la réponse
            Log::info('MOYOO Firebase Notification Sent', [
            ...
            Log::error('MOYOO Firebase Notification Error', [
                'project_id' => $this->projectId,
                'error' => $e->getMessage(),
                'payload' => $payload
            ]);
