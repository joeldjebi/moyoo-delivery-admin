        ...
        <div class="col-lg-12 mb-4 order-0">
            <div class="card">
                <div class="row">
                    <div class="col-sm-7">
                        <div class="card-body">
                            <h5 class="card-title text-primary">Modifier le Tarif de Livraison</h5>
                            <p class="mb-4">Modifiez les informations du tarif</p>
                        </div>
                    </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('commune_id') is-invalid @enderror" id="commune_id" name="commune_id" required>
                                    ...
                                    @foreach($communes as $commune)
                                        <option value="{{ $commune->id }}" {{ (old('commune_id', $tarif->commune_id) == $commune->id) ? 'selected' : '' }}>
                                            {{ $commune->libelle }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('type_engin_id') is-invalid @enderror" id="type_engin_id" name="type_engin_id" required>
                                    ...
                                    @foreach($typeEngins as $typeEngin)
                                        <option value="{{ $typeEngin->id }}" {{ (old('type_engin_id', $tarif->type_engin_id) == $typeEngin->id) ? 'selected' : '' }}>
                                            {{ $typeEngin->libelle }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('mode_livraison_id') is-invalid @enderror" id="mode_livraison_id" name="mode_livraison_id" required>
                                    ...
                                    @foreach($modeLivraisons as $modeLivraison)
                                        <option value="{{ $modeLivraison->id }}" {{ (old('mode_livraison_id', $tarif->mode_livraison_id) == $modeLivraison->id) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('poids_id') is-invalid @enderror" id="poids_id" name="poids_id" required>
                                    ...
                                    @foreach($poids as $poid)
                                        <option value="{{ $poid->id }}" {{ (old('poids_id', $tarif->poids_id) == $poid->id) ? 'selected' : '' }}>
                                            {{ $poid->libelle }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('temp_id') is-invalid @enderror" id="temp_id" name="temp_id" required>
                                    ...
                                    @foreach($temps as $temp)
                                        <option value="{{ $temp->id }}" {{ (old('temp_id', $tarif->temp_id) == $temp->id) ? 'selected' : '' }}>
                                            {{ $temp->libelle }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <p><strong>Créé le :</strong> {{ $tarif->created_at->format('d/m/Y à H:i') }}</p>
                            <p><strong>Dernière modification :</strong> {{ $tarif->updated_at->format('d/m/Y à H:i') }}</p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>Créé par :</strong> {{ $tarif->createdBy->first_name ?? 'N/A' }} {{ $tarif->createdBy->last_name ?? 'N/A' }}</p>
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        console.log('Preview updated:', { commune, typeEngin, modeLivraison, poids, temp, amount });
    }
