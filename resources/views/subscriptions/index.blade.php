                    ...
                    <div class="card-body pb-0 px-0 px-md-4">
                        <div class="text-end">
                            ...
                            @if($user->hasActiveSubscription() && $activeSubscription)
                                <span class="badge bg-label-{{ in_array($activeSubscription->name, ['Premium', 'Premium Annuel']) ? 'primary' : 'secondary' }}">
                                ...
                                @elseif($activeSubscription->expires_at)
                                    <br><small class="text-muted">Expire le {{ $activeSubscription->expires_at->format('d/m/Y') }}</small>
                                @endif
                            @else
                                <span class="badge bg-label-secondary">Aucun abonnement</span>
                                <br><small class="text-muted">Aucun abonnement actif</small>
                            @endif
                        </div>
                    </div>
                </div>
...
<div class="row">
    @foreach($plans as $plan)
        <div class="col-lg-6 col-xl-4 mb-4">
            <div class="card h-100 {{ $user->hasActiveSubscription() && $user->subscription_plan_id == $plan['id'] ? 'border-primary' : '' }}">
                <div class="card-header text-center">
                    ...
                    <div class="mt-3">
                        ...
                        <div class="text-muted small">par {{ $plan['period'] }}</div>
                    </div>
                </div>
                <div class="card-body">
                ...
                <div class="card-footer">
                    ...
                    @elseif($plan['name'] === 'Free' && $user->hasActiveSubscription())
                    ...
                    @else
                        ...
                        @else
                            <a href="{{ route('subscriptions.payment', $plan['id']) }}" class="btn btn-{{ $plan['popular'] ? 'primary' : 'outline-primary' }} w-100">
...
<!-- Informations sur l'abonnement actuel (seulement si l'utilisateur a un abonnement actif) -->
@if($user->active_subscription_plan)
    <div class="row mt-4">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <h6>Plan: {{ $user->active_subscription_plan->name }}</h6>
                            <p class="text-muted">{{ $user->active_subscription_plan->description }}</p>
                        </div>
                        <div class="col-md-6">
