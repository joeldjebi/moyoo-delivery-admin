...
# Script simple pour créer une nouvelle API
# Usage: ./scripts/simple-api.sh "ColisAssignes" "colis-assignes" "Colis Assignés"

CLASS_NAME="$1"
ROUTE_NAME="$2"
API_DESCRIPTION="$3"

if [ -z "$CLASS_NAME" ] || [ -z "$ROUTE_NAME" ] || [ -z "$API_DESCRIPTION" ]; then
    echo "❌ Usage: $0 \"ColisAssignes\" \"colis-assignes\" \"Colis Assignés\""
    exit 1
fi

CONTROLLER_NAME="${CLASS_NAME}Controller"

echo "🚀 Création d'une nouvelle API: $API_DESCRIPTION"
echo "📁 Contrôleur: $CONTROLLER_NAME"
echo "🛣️ Route: $ROUTE_NAME"
...
echo "    Route::get('$ROUTE_NAME', [${CONTROLLER_NAME}::class, 'index']);" >> routes/api.php
echo "});" >> routes/api.php
